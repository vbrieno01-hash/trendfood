...
// Cache em memória respeitando rate-limit de 30s/order do iFood.
const TRACKING_CACHE = new Map<string, { fetched_at: number; data: any; http: number }>();
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let ifoodOrderId = url.searchParams.get("ifood_order_id") || "";
    if (req.method === "POST") {
      try {
        const body = await req.json();
        orgId = orgId || String(body?.organization_id || "");
        ifoodOrderId = ifoodOrderId || String(body?.ifood_order_id || "");
      } catch (_) {}
    }
    if (!orgId || !ifoodOrderId) {
      return new Response(JSON.stringify({ error: "organization_id and ifood_order_id required" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const cacheKey = `${orgId}::${ifoodOrderId}`;
    const cached = TRACKING_CACHE.get(cacheKey);
    if (cached && Date.now() - cached.fetched_at < CACHE_TTL_MS) {
      return new Response(JSON.stringify({ ...cached.data, cached: true, age_ms: Date.now() - cached.fetched_at }),
    ...
    const res = await fetch(`${IFOOD_API}/order/v1.0/orders/${ifoodOrderId}/tracking`, {
    ...
    if (res.ok) {
      TRACKING_CACHE.set(cacheKey, { fetched_at: Date.now(), data: parsed, http: res.status });
    }

    return new Response(JSON.stringify({ http: res.status, ...parsed, cached: false }),
      { status: res.ok ? 200 : res.status, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err: any) {
    console.error("[ifood-tracking]", err);

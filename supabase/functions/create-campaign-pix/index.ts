...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── DEDUP: se já existe um PIX pending recente pra essa org, reaproveita
    ...
    try {
      ...
      if (recentPending?.payment_id) {
        ...
        if (chkRes.ok) {
          ...
          if (chkData.status === "pending") {
            ...
            // Marca como failed pra não voltar aqui na próxima tentativa
            await admin
              .from("pending_subscription_payments")
      ...
      console.error("[create-campaign-pix] dedup lookup (non-blocking):", e);
    }

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch(`${IFOOD_API}/order/v1.0/orders/${ifoodOrderId}/verifyDeliveryCode`, {
      method: "POST",
    ...
    await supabase.from("ifood_event_log").insert({
      ...
      code: res.ok ? "OUT_DELIVERY_CODE_VALID" : "OUT_DELIVERY_CODE_INVALID",
    ...
    console.error("[ifood-verify-delivery-code]", err);

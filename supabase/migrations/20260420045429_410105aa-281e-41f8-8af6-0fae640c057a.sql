...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Organização não encontrada.' USING ERRCODE = 'P0001';
  END IF;

  IF _org.paused THEN
    RAISE EXCEPTION 'Loja pausada no momento. Pedidos não podem ser feitos.' USING ERRCODE = 'P0001';
  END IF;
  ...
  IF _org.force_open THEN
    ...
    THEN
      ...
      IF _current_minutes >= _break_from_min AND _current_minutes < _break_to_min THEN
        RAISE EXCEPTION 'Estamos em pausa. Voltamos às %.', _today->>'break_to' USING ERRCODE = 'P0001';
      END IF;
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF NOT _is_open THEN
    RAISE EXCEPTION 'Loja fechada no momento. Pedidos só podem ser feitos no horário de funcionamento.' USING ERRCODE = 'P0001';
  END IF;
  ...
  THEN
    ...
    IF _current_minutes >= _break_from_min AND _current_minutes < _break_to_min THEN
      RAISE EXCEPTION 'Loja em intervalo de descanso. Retorna às %.', _active_day->>'break_to' USING ERRCODE = 'P0001';
    END IF;
  END IF;

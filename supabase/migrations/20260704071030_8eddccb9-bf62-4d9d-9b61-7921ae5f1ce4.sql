...
BEGIN
  ...
  SELECT value INTO v_token FROM public.platform_secrets WHERE key = 'fiscal_internal_token';
  IF v_url IS NULL OR v_anon IS NULL OR v_token IS NULL THEN
    RETURN NEW;
  END IF;

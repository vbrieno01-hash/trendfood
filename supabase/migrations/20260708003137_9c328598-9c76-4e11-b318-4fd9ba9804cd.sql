...
-- Atualiza apenas orgs que ainda têm o texto padrão antigo salvo (não sobrescreve customizações)
UPDATE public.organizations
SET wa_auto_status = jsonb_set(
  wa_auto_status,
  '{templates,new_order_customer}',
  to_jsonb('Olá, {nome}! 👋 Recebemos seu pedido #{numero} no valor de *R$ {total}*. Ele está aguardando a confirmação do estabelecimento! 📝 — {loja}'::text)
)
WHERE wa_auto_status ? 'templates'
  AND wa_auto_status->'templates'->>'new_order_customer'
...
SET wa_auto_status = jsonb_set(
  wa_auto_status,
  '{templates,preparing}',
  to_jsonb(E'Seu pedido #{numero} foi aceito e já entrou em preparação na nossa cozinha! 🍳\n💰 Total: *R$ {total}* — {loja}'::text)
)
WHERE wa_auto_status ? 'templates'
  AND wa_auto_status->'templates'->>'preparing'

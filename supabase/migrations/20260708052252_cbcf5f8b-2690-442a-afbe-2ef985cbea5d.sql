...
-- 1) Guarda dentro de wa_enqueue_status: nunca enfileira new_order_* com total zero.
...
BEGIN
  ...
  -- (será re-enfileirado pelo trigger de order_items assim que o primeiro item chegar).
...
-- 2) Trigger em order_items: dispara new_order_customer e new_order_owner
--    APÓS os itens já existirem, garantindo total correto. Dedupe já existente
--    em wa_enqueue_status impede duplicação para pedidos com múltiplos itens.
CREATE OR REPLACE FUNCTION public.tg_order_items_wa_new_order()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  BEGIN
    SELECT organization_id INTO v_org_id FROM orders WHERE id = NEW.order_id;
    IF v_org_id IS NULL THEN
      RETURN NEW;
    END IF;
    PERFORM wa_enqueue_status(v_org_id, NEW.order_id, 'new_order_customer');
    PERFORM wa_enqueue_status(v_org_id, NEW.order_id, 'new_order_owner');
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'tg_order_items_wa_new_order suprimido order=% err=% state=%',
      COALESCE(NEW.order_id::text,'nil'), SQLERRM, SQLSTATE;
  END;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_order_items_wa_new_order ON public.order_items;
CREATE TRIGGER trg_order_items_wa_new_order
AFTER INSERT ON public.order_items
FOR EACH ROW EXECUTE FUNCTION public.tg_order_items_wa_new_order();

...
export default function WaiterPage() {
  ...
  const [orgExtra, setOrgExtra] = useState<{
    ...
    pix_confirmation_mode?: string | null;
    pix_key?: string | null;
  }>({});
  ...
  useEffect(() => {
    ...
    supabase
      .from("organizations")
      .select("whatsapp, pix_confirmation_mode, pix_key")
      .eq("id", org.id)
      .maybeSingle()
      .then(({ data }) => {
        if (data) setOrgExtra(data as any);
      });
  }, [org?.id]);
  ...
  const pixConfirmationMode = orgExtra.pix_confirmation_mode;
  const pixKey = orgExtra.pix_key;
  const whatsapp = orgExtra.whatsapp ?? org?.whatsapp;
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {unpaidOrders.length > 0 && ` · ${unpaidOrders.length} conta${unpaidOrders.length !== 1 ? "s" : ""} aberta${unpaidOrders.length !== 1 ? "s" : ""}`}
              </p>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto p-4 max-[380px]:p-3 md:p-6 space-y-8">
                  ...
                  >
                    ...
                    <div className="flex gap-2 pt-1">
                      <Button
                        ...
                        onClick={() => {
                          const pix = pixKey && total > 0 ? buildPixPayload(pixKey, total, org?.name ?? "LOJA") : undefined;
                          printOrder(order, org?.name, pix);
                        }}
               ...
               <div className="flex justify-center mb-3">
                <div className="animate-[float_3s_ease-in-out_infinite]">
                  <svg viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-16 h-16">
                    <defs>
                      <radialGradient id="paidGlow2" cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stopColor="hsl(142 71% 45% / 0.2)" />
                        <stop offset="100%" stopColor="transparent" />
                      </radialGradient>
                    </defs>
                    <circle cx="60" cy="60" r="56" fill="url(#paidGlow2)" className="animate-[pulse_3s_ease-in-out_infinite]" />
                    <circle cx="60" cy="60" r="36" stroke="hsl(142 71% 45%)" strokeWidth="3" fill="hsl(142 71% 45% / 0.1)" />
                    <path d="M42 60 L54 72 L78 48" stroke="hsl(142 71% 45%)" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none" style={{ strokeDasharray: 60, strokeDashoffset: 60, animation: 'paidCheck2 0.6s ease-out 0.3s forwards' }} />
                  </svg>
                  <style>{`
                    @keyframes paidCheck2 { to { stroke-dashoffset: 0; } }
                  `}</style>
                </div>
              </div>

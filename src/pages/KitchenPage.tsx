...
export default function KitchenPage() {
          ...
          if (status === "ready") {
            ...
            if (order) {
              ...
              if (phone) {
                ...
                notifyCustomerReady(phone, (order as any).order_number || order.id.slice(0, 6), org?.name, order.notes, reviewUrl);
              }
            }
          }
        },
  ...
  useEffect(() => {
    ...
    if (pendingWithoutItems.length > 0 && toPrint.length === 0) {
      setTimeout(() => qc.invalidateQueries({ queryKey: ["orders", org?.id, ["pending", "preparing"]] }), 2000);
      return;
    }

...
const UnitPage = () => {
  ...
  // Store open/closed status (uses _tick from top-level useState for auto-refresh)
   ...
   try {
    ...
    const lines = [
      ...
      ...cartItems.map((i) => {
        ...
        if (i.addons.length > 0) line += ` (${i.addons.map(a => `+ ${a.qty > 1 ? `${a.qty}x ` : '1x '}${a.name} R$${(a.price * a.qty).toFixed(2).replace('.', ',')}`).join(", ")})`;
      ...
      placeOrder.mutate(
        ...
        },
        {
          onSuccess: (order) => {
            ...
            // A notificação ao dono é enfileirada automaticamente pelo trigger
            // SQL `trg_orders_wa_auto_status` (via wa_enqueue_status) na inserção
            // do pedido. Chamar `uazapi-notify-owner` aqui gerava mensagem duplicada.
            ...
            {isClosed ? (
              ...
              showPixScreen && pixOrderId ? (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3.5 text-white font-semibold text-sm transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-60 disabled:pointer-events-none"
